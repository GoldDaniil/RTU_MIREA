...
// Высота горной пирамиды (можно менять)
const int mountain_height = 4;

// Функция для установки одной горы (пирамидой)
bool placeMountainAt(Animal grid[][screen_width], int startX, int startY) {
    // Проверка, влезет ли гора
    if (startX + mountain_height >= screen_height || startY - mountain_height < 0 || startY + mountain_height >= screen_width)
        return false;

    // Проверка, заняты ли клетки
    for (int i = 0; i < mountain_height; ++i) {
        int row = startX + i;
        int left = startY - i;
        int right = startY + i;
    ...
    // Установка символов горы
    for (int i = 0; i < mountain_height; ++i) {
        int row = startX + i;
        int left = startY - i;
        int right = startY + i;
        for (int col = left; col <= right; ++col) {
            grid[row][col].symbol = mountain_symbol;
        }
    }
...
// Основная функция для размещения нескольких гор
void placeRandomMountains(Animal grid[][screen_width], int mountainCount) {
    int placed = 0;
    int attempts = 0;

    while (placed < mountainCount && attempts < 1000) {
        int x = rand() % (screen_height - mountain_height);
        int y = mountain_height + rand() % (screen_width - mountain_height * 2); // центр горы

        if (placeMountainAt(grid, x, y)) {
            ++placed;
        }

        ++attempts; // чтобы не зависло в бесконечном цикле
    }
}
