...
void generate_huffman_codes(huffman_node* node, string current_code, map<char, string>& huffman_codes) {
    ...
    generate_huffman_codes(node->left_child, current_code + "0", huffman_codes);
...
int main() {
    ...
    priority_queue<huffman_node*, vector<huffman_node*>, compare> priority_queue_huffman_nodes;
    ...
    while (priority_queue_huffman_nodes.size() > 1) {
