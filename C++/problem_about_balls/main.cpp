...
void generate_permutation(int permutation[], int m, int n, int i, int& count) {
    if (i == n) {
    ...
    if (i == m) {
        ...
        for (int j = i; j < n; j++) {
            ...
            generate_permutation(permutation, m, n, i + 1, count);
            swap(permutation[i], permutation[j]);
        }
    }
}
...
int main() {
    ...
    generate_permutation(balls, n, n, 0, count);

    cout << "\ntotal number of permutations: " << count << endl;
