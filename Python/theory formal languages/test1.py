class TaskOne:
    ...
    def is_valid_expression(expression):
        # проверка - выражение состоит только из допустимых символов (цифры, буквы, операторы, скобки)
    ...
    def infix_to_rpn(expression):
        ...
        for char in expression:
            if char.isalnum():  # теперь разрешены цифры и буквы
                output.append(char)
            elif char == '(':
                stack.append(char)
            elif char == ')':
    ...
    @staticmethod
    def evaluate_rpn(rpn, variables):
        stack = []
        for char in rpn:
            if char.isdigit():
                ...
                if char in variables:
                    ...
                    print(f"{Colors.RED}ошибка: переменная '{char}' не определена{Colors.RESET}")
                    return None
            else:
                b = stack.pop()
                a = stack.pop()
                if char == '+':
                    ...
                    if b == 0:
                        print(f"{Colors.RED}нельзя делить на 0!{Colors.RESET}")
                        return None
                    stack.append(a // b)
        return stack[0]
    ...
    def run():
        while True:
            ...
            try:
                ...
                # сбор значений для переменных
                variables = {}
                for char in set(expression):
                    if char.isalpha():  # если это переменная (буква)
                        value = input(f"введите значение для переменной '{char}': ")
                        if value.isdigit():
                ...
                result = TaskOne.evaluate_rpn(rpn, variables)
