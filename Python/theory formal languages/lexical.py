...
def lexical_analysis():
    ...
    #output_symbol = get_unique_input("введите символ, который будет выходным - например : '}': ", used_values)
    ...
    #m1_symbols = get_unique_input("введите символы для M1 - например : '< <= !=': ").split()
    #m2_symbols = get_unique_input("введите символы для M2 - например : '> >=': ").split()

    #operations_input = get_unique_input("введите символы для группы операций (сложение вычитание дизъюнкция): ")
    #operations_list = operations_input.split()

    output_symbol = input("введите символ, который будет выходным - например : '}': ")
    comment_start = input("введите символ начала комментария -н апример : '/': ")
    comment_end = input("введите символ конца комментария - например : '*': ")
    transition_symbol = input("введите связующий символ между C1, C2 и C3 = например : 'A': ")
    m1_symbols = input("введите символы для M1 - например : '< <= !=': ").split()
    m2_symbols = input("введите символы для M2 - например : '> >=': ").split()

    operations_input = input("введите символы для группы операций (сложение вычитание дизъюнкция): ")
    operations_list = operations_input.split()

    if len(operations_list) < 3:
        print("ошибка: введите как минимум три символа (сложения вычитания дизъюнкции)")
        return

    operation_tokens = {
        'сложение': operations_list[0],
        'вычитание': operations_list[1],
        'дизъюнкция': operations_list[2]
    }
    ...
    operations_input_multiplication = input("введите символы для группы операций (умножение, деление, конъюнкция): ")
    operations_list_multiplication = operations_input_multiplication.split()
    if len(operations_list_multiplication) < 3:
        print("ошибка: введите как минимум три символа (умножение, деление, конъюнкция)")
        return

    operation_tokens_multiplication= {
        'умножение': operations_list_multiplication[0],
        'деление': operations_list_multiplication[1],
        'конъюнкция': operations_list_multiplication[2]
    }
