...
task4() {
    echo "Выполняется задание 4"
    grep -o '\b[A-Za-z_][A-Za-z0-9_]*\b' hello.c | sort -u
}

task5() {
    echo "Выполняется задание 5"
    if [ -z "$1" ]; then
      echo "Укажите имя программы для регистрации."
      exit 1
    fi
...
while true; do
    echo "
    Выберите задание (от 1 до 8) или нажмите 'q' для выхода:"
    read -r choice
